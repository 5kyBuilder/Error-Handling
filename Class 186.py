...
ycode, fcode, name, text, entry = "", "", "", "", ""

def sendData():
    global name, ycode, entry
    
    message = name + " " + entry.get()
    ciphercode = encrypt("AIM", message)
    hex_string = ciphercode.hex()
    
    put_data = firebase.put("/", ycode, hex_string)
    print(put_data)

def enterRoom():
    global ycode, fcode, name, text, entry
    
    ycode = your_code_entry.get()
    fcode = friends_code_entry.get()
    ...
    entry = Entry(message_window, font = 'arial 15')
    entry.place(relx=0.6,rely=0.8, anchor=CENTER)
